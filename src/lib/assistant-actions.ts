...
Mera Khata: tap the round + button to log an expense (amount, paid to, category, note, date and time). Categories fill in automatically from the payee. Month / Year switch with arrows to move between periods. The expenses for the period come first, then "Where it went" with the top categories (Show all for the rest). The "All categories" button above the list opens a picker of every category with its total - pick one to filter, Clear to go back; the search button searches amount, note, vendor or category. Tap any expense to see it, edit it or delete it. Expenses without a category show a prompt to sort them. The ... menu has Manage categories (add, rename, delete, keywords that auto-match), Re-categorise (preview and apply category fixes) and Download Excel.
