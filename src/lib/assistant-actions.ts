...
Tabs (bottom bar): Home, Khata (Mera Khata), Udhar (Udhar Khata), Subs (Subscriptions), and Trips when it is switched on in Settings. Accounts with assistant access also have the assistant button in the middle. The bell (top right, beside the profile picture) lists every notification that has been sent, newest first, with a badge for unseen ones; tapping one opens what it is about. The profile picture opens: Settings, Admin (admins only), Log out. Settings holds your profile name, appearance (Light, Dark or System), the Trips switch and "Manage notifications".
...
Trips (switch it on in Settings > Sections > Trips): splitting a group trip. Start a trip with a name, optional dates and the names of whoever is going - you are on it already. Everyone's deposits make the pot, and what is in the pot is the budget. Log spending as it happens: each expense is paid either from the pot or by one person out of their own pocket, and is split equally between whoever is ticked on it (everyone by default). Three tabs inside a trip: Spending, People (what each person put in, their share and where they stand) and Settle up (the fewest payments that square everyone up, kept up to date as you go). "Close trip and settle up" fixes that list so payments can be ticked off, and offers to add your share of the trip to Mera Khata as one expense and to send what is still owed between you and the others to Udhar Khata. A closed trip can be reopened, which takes that expense back out. Trip spending never appears in Mera Khata until the trip is closed, because most of it is other people's money.

Siri and Shortcuts (Settings > Siri & Shortcuts, accounts with assistant access): make a token, then build a Shortcut on the iPhone with Dictate Text, Get Contents of URL (POST to /api/shortcut, an Authorization header holding the token, a JSON body with a "text" field set to the dictated text), Get Dictionary Value for "reply" and Speak Text. Naming the Shortcut "Khata" means "Hey Siri, Khata" starts it, and anything that can be typed to the assistant can be said out loud, including undo. The step-by-step is on that screen. Making a new token stops every Shortcut holding the old one; Turn off ends it altogether. Siri cannot reach Khata any other way, as it is a web app rather than an App Store app.
...
Reminders (every account): Settings > Manage notifications. "Notify this device" registers the phone or computer you are on - each device is switched on separately, and on an iPhone the app has to be on the Home Screen first, as Apple does not allow notifications from a Safari tab. Under "What to send", each kind has its own switch: the missed-expense reminder, new expenses from your bank, subscriptions due, Udhar follow-ups and the monthly summary. At 4am: a reminder to add anything paid yesterday that was not logged (cash, or anything the bank alerts miss); tapping it opens a new expense. New expenses from your bank: sent when expenses are added automatically from bank alert emails - one notification per batch, plus a separate one for each expense that could not be categorised, which opens that expense so a category can be chosen. At 6pm: a subscription the day before it is due and on the due day if still unpaid, and a person on their follow-up date. On the 1st: last month's summary. Every notification also lands in the bell at the top of the app, and the app icon shows how many are unread. If a phone cannot be reached, Khata keeps retrying for a while, and Settings shows when this device last received one, with a Reconnect button if delivery stopped. Each notification opens the record it is about, and they arrive with the app closed. Khata sends no email at all.
